...
const MANAGER_SYSTEM =
  ...
  'CRITICAL RULE — POSTING PLANS: If the member asks for a rehearsal plan / schedule / what to practice / how to prepare, you MUST call the show_plan tool (do not answer with the plan only as chat text; call show_plan FIRST, before your text reply). ' +
  'A plan is an ORDERED, UNDATED LIST of rehearsals: title each session ONLY as "Rehearsal 1", "Rehearsal 2", … in order. NEVER attach a date, time, weekday or R-number to a session — the band assigns real dates in the app afterwards. ' +
  'Decide HOW MANY rehearsals to include from the owner guidelines + time left until the show: take the rehearsals-per-week cadence × weeks remaining, subtract the expected cancellation rate, and add the extra rehearsals the guidelines call for near the show. So if the guideline is 2/week for ~10 weeks with ~20% cancellations plus a final-week ramp, produce roughly that many rehearsals — one session each. Never collapse them into ranges or "ongoing weekly". ' +
  'Every session must name SPECIFIC song titles from the setlist in learn/practice — never "4-5 songs" or "rotating". Choose each session\'s songs from readiness, what earlier sessions already covered, and which docs/stems are missing; move from learning weak/new songs early to full run-throughs near the show. ' +
  'Also set the plan\'s risk level (none/low/medium/high) from how much SPARE capacity exists: comfortably more rehearsals than the work needs = none/low, just barely enough = medium, not enough time or already behind = high. It is fine for the plan to be long. After calling show_plan a one-line text reply is enough — NEVER just describe a plan in prose and stop. You have the full band data below — use it. ' +
...
const MANAGER_TOOLS = [
  ...
  { name: 'show_plan', description: 'Post a rehearsal plan to the app (the "Rehearsal plan" panel). The plan is an ORDERED, UNDATED list of rehearsals — the band assigns real dates later. Write song names and docs as plain text.', input_schema: { type: 'object', properties: { summary: { type: 'string', description: 'One or two lines of overview' }, risk: { type: 'string', enum: ['none', 'low', 'medium', 'high'], description: 'How risky hitting the show is, from how much SPARE rehearsal capacity exists vs the work left: none/low = comfortable buffer, medium = tight, high = not enough time / behind.' }, sessions: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { title: { type: 'string', description: 'Ordinal only, NO date: "Rehearsal 1", "Rehearsal 2", …' }, learn: { type: 'array', items: { type: 'string' } }, practice: { type: 'array', items: { type: 'string' } }, docs: { type: 'array', items: { type: 'string' }, description: 'Docs to prepare, e.g. "slide for Black Bird"' }, note: { type: 'string' } }, required: ['title'] } } }, required: ['sessions'] } },
...
function applyManagerTool(name, input, ctx, showId, work, ops) {
  ...
  if (name === 'save_guideline') {
    ...
    const risk = ['none', 'low', 'medium', 'high'].indexOf((input.risk || '').toLowerCase()) >= 0 ? input.risk.toLowerCase() : 'none';
    op = { type: 'plan', plan: { summary: input.summary || '', risk, sessions, updatedAt: Date.now() } };
    msg = 'Posted the rehearsal plan to the app (' + sessions.length + ' rehearsal' + (sessions.length === 1 ? '' : 's') + ', risk: ' + risk + ').';
