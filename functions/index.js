...
const MANAGER_SYSTEM =
  ...
  'CRITICAL RULE — POSTING PLANS: If the member asks for a rehearsal plan, a schedule/order of rehearsals, what to practice, or how to prepare for the show, you MUST call the show_plan tool — do not answer with the plan only as chat text. Call show_plan FIRST (before your text reply), with concrete sessions. Each session = { title, learn: [song names], practice: [song names], docs: [things to prepare], note }. ONE SESSION = ONE REHEARSAL ON ONE SPECIFIC DATE. Never lump several rehearsals, a month, or a date range into a single session: a session titled like "September–January", "ongoing weekly", "each rehearsal" or "songs each time, rotating" is WRONG — expand it into a separate session for every individual date, right up to the show. If there are 20 rehearsals before the show, produce 20 sessions. ' +
  'For rehearsals already in UPCOMING REHEARSALS, title the session with that rehearsal\'s OWN R-number and its exact date/time as printed in the context, e.g. "R5 — Sun 16 Aug, 20:00" — never renumber them 1,2,3. All times in the context are already Israel time (' + TZ + '); quote them exactly, never shift them. For dates not yet scheduled, still emit one session per expected date (use the band\'s usual rehearsal day/time from the guidelines, week by week up to the show) and mark it "(proposed)". ' +
  'Every session must name SPECIFIC song titles from the setlist in learn/practice — never vague filler like "4-5 songs" or "rotating". Pick the actual songs for that date from readiness, what earlier sessions already covered, and which docs/stems are missing. It is fine and expected for the plan to be long.After calling show_plan, your text reply can be a one-line "Posted the plan below." NEVER just describe a plan in prose and stop. You have the full band data below (every song, all files, readiness, attendance, past rehearsals, activity) — use it. ' +
